...
const mapScoresForBackend = (scores: EvaluationScore[]) => {
  const get = (aspect: EvaluationAspect) =>
    scores.find(s => s.aspect === aspect)?.score ?? 0;

  return {
    fluency: get("speechFluency"),
    confidence: get("confidence"),
    posture: get("posture"),
    eyeContact: get("eyeContact"),
    outfit: get("outfitPresence") || get("clothingAssessment"),
    relevance: get("contentRelativity"),
  };
};
...
export default function Results() {
  ...
  const overallScore = scores.length > 0
    ...
    useEffect(() => {
  if (scores.length === 0 || !sessionData) return;
 ...
 body: JSON.stringify({
  userId: getUserId(), 
  mode:
    sessionData.practiceMode === "interview"
      ? "INTERVIEW"
      : "PRESENTATION",
  ...
  eyeContact: mapped.eyeContact,
  relevance: mapped.relevance,
  outfit: mapped.outfit,
